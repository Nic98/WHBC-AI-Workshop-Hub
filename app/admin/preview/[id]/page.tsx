...
export default async function AdminProjectPreview({ params }: { params: Promise<{ id: string }> }) {
  ...
  const source = project.sourceType === "url" ? project.externalUrl : `/embed/${project.id}/${versionId}/index.html`;
  return (
    <main className="preview-shell">
      ...
      <section className="runner-viewport">
        {project.sourceType === "url" && project.embedMode === "external" ? (
          ...
          <iframe allow="camera 'none'; microphone 'none'; geolocation 'none'; payment 'none'; usb 'none'" sandbox={project.sourceType === "url" ? "allow-scripts allow-forms allow-downloads allow-modals allow-same-origin" : "allow-scripts allow-forms allow-downloads allow-modals"} src={source!} title={`Preview ${project.title}`} />
        )}
      </section>
    </main>
  );
